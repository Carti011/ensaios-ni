...
- **Exportador** — rotina que escreve a série temporal do ensaio num formato de saída: **CSV** legível (hoje), **TXT** para o AqDAnalysis (a implementar) e **Excel** — CSV amigável ao Excel BR (separador `;`, decimal vírgula) e/ou **`.xlsx`** nativo (via `openpyxl`), porque o dono valoriza muito abrir os dados no Excel. Permite escolher **quais sinais** exportar. Extensão ≠ formato: interoperar é **converter o conteúdo**, não renomear arquivo. Ver [ADR-011](docs/adr/011-estrategia-de-exportacao.md).
