...
- **FlexLogger** — aplicação **paga (assinatura)** da NI para configurar ensaios, registrar e calibrar. **É o que este projeto substitui** e a **referência funcional de paridade**: replicamos seu modelo mental (escala, zero, clip) com UX própria. Ver [ADR-008](docs/adr/008-paridade-funcional-flexlogger.md) e [referencia-flexlogger.md](docs/referencia-flexlogger.md). O dono paga por ela hoje.
...
- **AqDados (Lynx)** — software de **aquisição** multicanal do dono (config de canais, **calibração de sensores**, gravação em tempo real com taxas elevadas). Referência funcional direta da **calibração por pontos** (ADR-006). Versão citada no site: **7.2**.
- **AqDAnalysis (Lynx)** — software de **análise** do dono (domínio do tempo/frequência, **filtros digitais**, relatórios técnicos). Referência da análise/FFT (futura) e da **remoção de outliers** (pós-processamento). Compatibilizar formato de arquivo é meta.
